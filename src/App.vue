	...
	<div class="m-auto container">
		<div class="flex min-h-full	">
			...
			<div class="w-1/3 m-auto p-10">
				...
				<input class="p-4 w-full bg-orange-300 hover:bg-orange-400 rounded-lg text-center text-gray-800" v-model="address" placeholder="Enter Bitcoin address" />

				<div class="my-5"></div>

				<p class="text-xl font-bold bg-orange-300 p-3 rounded-lg text-center">BTC : {{ balance / 10 ** 8 }}</p>


				<button class="mt-5 w-full bg-orange-300 px-4 py-3 rounded-lg text-bold hover:bg-orange-400" @click="getBalance" v-if="!bossmanBal">FETCH</button>
			</div>

			<div class="w-1/3 m-auto p-10">
				<div class="text-center max-w-lg mx-auto">
					<p class="mb-7 text-lg text-gray-600 font-medium" v-if="balance !== null">Balance: {{ (balance / 10 ** 8) * bitcoinPrice }} USD</p>

					<p class="mb-7 text-lg text-gray-600 font-medium" v-if="bitcoinPrice !== null">Bitcoin Price: {{ bitcoinPrice }}</p>
				</div>
			</div>
		</div>
	</div>
